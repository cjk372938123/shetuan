...
public class InviteAdapter extends BaseAdapter {
    ...
    public void refresh(List<InvationInfo> invationInfos)
    {
        ...
        {
            mInvitationInfos.clear();//防止多次添加对方出现多个添加好友申请
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        //2.获取当前item数据
        final InvationInfo invitationInfo = mInvitationInfos.get(position);
        ...
        if (user != null) {//联系人
            ...
            //除了添加好友, 其他原因不需要有接受和拒绝按钮
            holder.accept.setVisibility(View.GONE);
            holder.reject.setVisibility(View.GONE);

            //原因
            if (invitationInfo.getStatus() == InvationInfo.InvitationStatus.NEW_INVITE) {//新的邀请
            ...
            else if (invitationInfo.getStatus() == InvationInfo.InvitationStatus.INVITE_ACCEPT) {//接受邀请
                if (invitationInfo.getReason() == null) {
            ...
            else if (invitationInfo.getStatus() == InvationInfo.InvitationStatus.INVITE_ACCEPT_BY_PEER) {//邀请被接受
                if (invitationInfo.getReason() == null) {
    ...
    public interface OnInviteListener {
        //联系人接受按钮的点击事件
        void onAccept(InvationInfo invitationInfo);

        //联系人拒绝按钮的点击事件
        void onReject(InvationInfo invitationInfo);

        //接受邀请按钮处理
        void onInviteAccept(InvationInfo invitationInfo);

        //拒绝邀请按钮处理
        void onInviteReject(InvationInfo invitationInfo);

        //接受申请按钮处理
        void onApplicationAccept(InvationInfo invitationInfo);

        //拒绝申请按钮处理
        void onApplicationReject(InvationInfo invitationInfo);
    }
}
