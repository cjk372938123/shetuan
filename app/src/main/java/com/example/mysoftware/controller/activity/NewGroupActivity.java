...
public class NewGroupActivity extends Activity {
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if(resultCode == RESULT_OK){
            //创建群(String类型数组,所以后面是String类型的)
            //后面的key(members)要和上面的选择联系人key一致
    ...
    private void createGroup(final String[] members) {
        ...
        Model.getInstance().getGlobalThreadPool().execute(new Runnable() {
            ...
            public void run() {
                ...
                options.maxUsers = 100; //群最多容纳多少人
                options.inviteNeedConfirm = true;
                EMGroupManager.EMGroupStyle groupStyle =  EMGroupManager.EMGroupStyle.EMGroupStylePrivateMemberCanInvite;;
